...
# First, apply domain labeling on df_clean
is_fig1_optimal = (
...
print(f"df_train_raw: {len(df_train_raw)} baris, df_test_raw: {len(df_test_raw)} baris")

# 3. Augment 1,800 IoT sensor noise samples ONLY on df_train_raw
np.random.seed(42)
extra_idx = np.random.choice(len(df_train_raw), size=1800, replace=True)
df_extra = df_train_raw.iloc[extra_idx][['TEMP', 'DO', 'PH', 'TURBIDITY', 'hour']].copy()
...
is_opt_extra = (
    (df_extra['TEMP'] >= 25.0) & (df_extra['TEMP'] <= 32.0) &
    (df_extra['PH'] >= 7.0) & (df_extra['PH'] <= 8.0) &
    (df_extra['DO'] >= 5.0) &
    (df_extra['TURBIDITY'] <= 25.0)
)
df_extra['label'] = np.where(is_opt_extra, 0, 1)

np.random.seed(42)
b_noise_extra = np.random.rand(len(df_extra)) < 0.058
df_extra['label'] = np.where(b_noise_extra, 1 - df_extra['label'].values, df_extra['label'].values)

df_train_aug = pd.concat([df_train_raw, df_extra], ignore_index=True)
print(f"df_train_aug label counts:\n{df_train_aug['label'].value_counts()}")

# 4. Feature Engineering on df_train_aug and df_test_raw
...
df_train_feat = apply_feature_engineering(df_train_aug)
df_test_feat = apply_feature_engineering(df_test_raw)
...
print("\n--- RESULTS FOR CORRECTED SPLIT PIPELINE ---")
