...
def decision_engine(sensor, health_status, estimated_do):
    ...
    if (
        sensor.ph < settings["phMin"]
        or
        sensor.ph > settings["phMax"]
    ):
    ...
    if len(reasons) == 0:
        ...
        reasons.append(
            "All sensor parameters are within normal range."
        )
...
def analyze(sensor: SensorInput):
    ...
    # Update actuator state jika mode masih AUTO
    ...
    latest_result = {
        ...
        "water_circulation":
            decision["water_circulation"],

        "ph_neutralizer":
            decision["ph_neutralizer"],
    ...
    critical = (
        ...
        latest_result["health_status"] == "At Risk"
